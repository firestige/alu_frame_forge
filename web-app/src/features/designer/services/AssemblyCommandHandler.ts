...
export class AssemblyCommandHandler {
  ...
  private setupCommandListeners(): void {
    onCommand(
      'command:assembly:enter-constraint-mode',
      this.handleEnterConstraintMode
    );
    onCommand(
      'command:assembly:add-constraint-draft',
      this.handleAddConstraintDraft
    );
    onCommand(
      'command:assembly:set-active-constraint',
      this.handleSetActiveConstraint
    );
    onCommand(
      'command:assembly:place-connector-on-active-constraint',
      this.handlePlaceConnector
    );
  }

  private handleEnterConstraintMode = (
    data: EnterConstraintModeCommand
  ): void => {
    const { setConstraintMode } = useConstraintDraftStore.getState();
    setConstraintMode(data.enabled);

    console.log(
      `[AssemblyCommandHandler] Constraint mode ${data.enabled ? 'enabled' : 'disabled'}`
    );
  };

  private handleAddConstraintDraft = (
    data: AddConstraintDraftCommand
  ): void => {
    ...
    if (!anchorAId) {
      ...
      if (!objectA) {
        console.error(
          `[AssemblyCommandHandler] Object A not found: ${data.objectAId}`
        );
        return;
      }
      ...
      if (anchorsA.length === 0) {
        console.error(
          `[AssemblyCommandHandler] Object A has no anchors: ${data.objectAId}`
        );
        return;
      }
    ...
    if (!anchorBId) {
      ...
      if (!objectB) {
        console.error(
          `[AssemblyCommandHandler] Object B not found: ${data.objectBId}`
        );
        return;
      }
      ...
      if (anchorsB.length === 0) {
        console.error(
          `[AssemblyCommandHandler] Object B has no anchors: ${data.objectBId}`
        );
        return;
      }
    ...
    console.log(
      `[AssemblyCommandHandler] Draft constraint added: ${data.objectAId} <-> ${data.objectBId}`
    );
  };

  private handleSetActiveConstraint = (
    data: SetActiveConstraintCommand
  ): void => {
    const { setActiveDraft } = useConstraintDraftStore.getState();
    setActiveDraft(data.constraintDraftId);

    console.log(
      `[AssemblyCommandHandler] Active constraint set: ${data.constraintDraftId}`
    );
  };

  private handlePlaceConnector = async (
    data: PlaceConnectorCommand
  ): Promise<void> => {
    const { activeDraftId, drafts, selectedConnectorAssetId } =
      useConstraintDraftStore.getState();

    // Validation: Must have active draft
    if (!activeDraftId) {
      console.error(
        '[AssemblyCommandHandler] No active draft constraint selected'
      );
      return;
    }
    ...
    try {
      ...
      if (!connectorAsset) {
        console.error(
          '[AssemblyCommandHandler] Failed to load connector asset'
        );
        return;
      }
      ...
      const connectorObject = this.objectManager.createObjectFromAsset(
        connectorAssetId,
        {
      ...
      console.log(
        `[AssemblyCommandHandler] Connector object created: ${connectorObject.id}`
      );
      ...
      console.log(
        `[AssemblyCommandHandler] Constraint created: ${constraint.id}`
      );
      ...
      if (solveResult.success) {
        console.log(
          '[AssemblyCommandHandler] ✅ Connector placed successfully'
        );
        ...
        if (solveResult.solutions.length > 0) {
          ...
          console.log(
            `  Position: (${solution.position.x.toFixed(3)}, ${solution.position.y.toFixed(3)}, ${solution.position.z.toFixed(3)})`
          );
          console.log(
            `  Rotation: (${solution.rotation.x.toFixed(3)}, ${solution.rotation.y.toFixed(3)}, ${solution.rotation.z.toFixed(3)})`
          );
        }
      } else {
        ...
        if (solveResult.conflicts.length > 0) {
          solveResult.conflicts.forEach((conflict, i) => {
            console.log(
              `  Conflict ${i + 1}: ${conflict.constraintId} - ${conflict.reason}`
            );
          });
        }
      }
